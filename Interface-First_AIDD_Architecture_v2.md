...
#### **⑤ Vertical Slice Autonomy & Anti-DRY (垂直スライスの自律性と反DRY)**

*   **概念**: 機能単位（Vertical Slice）ごとにコンテキストを完全に独立させる。AIDDにおける決定的なコード再生成の確実性を担保するため、単一コンテキスト内での凝集度を高め、あえてプロジェクト全体にまたがるDRY原則（データ構造やDB操作ロジックの共通化）を捨てる。
*   **制約**: 各Sliceは「スキーマ定義」「DTO」「永続化ロジック」のすべての責務を自身で負う。依存として受け取るのはDB接続プールなどの純粋なインフラのみとし、外部機能（他のSlice）のデータモデルやDAOを使い回すことは厳禁とする。
*   **例外と判断基準 (WET vs DRY Flow)**:
    *   **技術的関心事**（DB接続、ユーティリティ等）は `Shared/Core` へDRYに集約する。
    *   **ドメインロジック**において、将来スライスごとに変化する可能性があるもの（または不明なもの）は **WET（コピペ許容）** を貫く。
    *   **ドメインロジック**において、将来的に「不変の真理」である純粋関数のみ例外的に `Domain Service` 等として共通化を許可する。
