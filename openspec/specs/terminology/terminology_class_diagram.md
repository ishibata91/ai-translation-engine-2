...
classDiagram
    ...
    class Pipeline {
        -Terminology termTranslator
        -JobQueue jobQueue
        +HandleTermTranslation(w http.ResponseWriter, r *http.Request)
    }

    Pipeline --> Terminology : uses
    Terminology <|.. TermTranslatorImpl : implements
...
スライス自身はLLM Clientを直接呼び出さず、呼び出し元のオーケストレーター（Pipeline）がリクエストの実行順序や並列度を制御する。
