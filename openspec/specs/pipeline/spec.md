...
現状、各垂直スライス（Terminology, Persona, Lore, Summary, Translator）は、モジュール間の密結合を防ぐために独自の入力用DTOを定義する設計（Anti-Corruption Layer）となっています。
ProcessManagerは、これらのスライスをつなぐハブとして機能します。

- **データ変換**: `parser_slice` 等から取得した共有データ（例: `ExtractedData`）を、各ドメインスライスが個別に要求する専用DTO（例: `TermTranslatorInput`）へ変換（マッピング）する責務を担います。
- **依存の遮断**: 各スライスが `parser_slice` のデータ構造に直接依存しないよう、プロセスマネージャーが中継することで、スライスの完全独立性を担保します。
...
### 3. メタデータ継承のマッピング管理
parser Sliceから上がってくる階層化されたデータ構造を翻訳リクエストに展開する際、必要なメタデータを確実に伝播させる。
