...
*   **外部辞書作成 (dictionary)**: xTranslator形式のXMLファイルを読み込み、名詞類（NPC名、アイテム名など）を抽出してSQLiteベースの用語辞書DBを構築する独立機能（Slice）を備えること。
...
*   **モデル明示指定**: LLM実行時は `provider` ごとのモデルIDを明示設定し、未指定時は実行エラーとすること（自動選択は行わない）。
*   **LM Studio互換**: `local` / `local-llm` 設定は後方互換として内部的に `lmstudio` へマップして扱うこと。
*   **進捗可視化**: 全体およびファイルごとの進捗状況をリアルタイムで表示すること。
*   **プレビュー機能**: 翻訳実行前に、生成されたコンテキストやAIへの指示内容を確認できること。
*   **設定管理**: エンジンの挙動に関する設定を変更できること。
*   **プロンプトテンプレート調整**: レコード種別（会話、武器、本など）ごとのシステムプロンプトテンプレートをUI上で閲覧・編集できること。変更内容はConfigに永続化され、翻訳エンジンが動的に参照すること。

### 3.5 エクスポート (exporter)
*   **配布形式出力**: Mod翻訳コミュニティで標準的な形式（JSON等）で出力すること。
*   **xTranslator互換性**: xTranslator互換のXML形式(SSTXMLRessources)を出力する exporter を備えること。
    *   **シナリオ: 翻訳出力の生成**: 翻訳プロセスが正常に完了した際、exporterを介してxTranslator互換のXML (SSTXMLRessources) を出力しなければならない。
...
### 4.2 信頼性・可用性
*   **逐次保存**: 翻訳が完了するごとに結果を保存し、不慮の停止時におけるデータ損失を最小限に抑えること。ただし、1件ごとのDB更新がボトルネックにならないよう、進捗の永続化についてはスロットリング等を用いて負荷を制御すること。
*   **エラー耐性**: 一部の翻訳が失敗しても、プロセス全体を停止せず、エラーを記録して処理を継続すること。
*   **ジョブ再開一貫性**: Queue再開時は保存済み `provider/model/request_fingerprint/structured_output_schema_version` を用いて同一条件で復元し、欠損時は再開失敗とすること。
*   **高頻度通知と一括保存**: 巨大なループ処理（辞書構築、翻訳等）において、フロントエンドへの進捗Emitは即時行い、DBへのデータ本体の書き込みはフェーズ完了時やバッチ単位での一括処理とすることで、N+1問題を回避し高い応答性を維持すること。
