...
## 概要
Lore Sliceの機能を統合し、ゲームデータから直接翻訳ジョブ（文脈を含むプロンプト）を構築・実行し、翻訳結果を出力する機能である。
本機能は2-Pass Systemにおける **Pass 2: 本文翻訳 (Main Translation)** の中核を担い、会話ツリー解析、話者プロファイリング、辞書検索、要約参照などの文脈構築から、LLMへのプロンプト生成、結果の保存までを垂直に一貫して担当する。
...
- **Loreスライスの統合**: 垂直スライス原則に基づき、文脈構築ロジックを本スライス内部に取り込み、オーケストレーションの複雑さを排除する。
- 要件定義書 §3.3「翻訳エンジン」に基づき、レコード種別に応じたプロンプト生成、タグ保護、リトライ/リカバリ、逐次保存を実現する。

## スコープ
### 本Sliceが担う責務
1. **翻訳ジョブの生成 (Phase 1: Propose)**: プロセスマネージャーからゲームデータ群を受け取り、内部の **Context Engine** による文脈構築（会話ツリー解析、話者プロファイリング、辞書検索、要約参照）、差分更新（Resume）チェック、タグ保護、プロンプト構築を行い、LLMリクエスト群（ジョブ）を構築して返す。
2. **翻訳結果の保存 (Phase 2: Save)**: LLMからのレスポンス群を受け取り、タグ復元、パース、バリデーション（タグハルシネーションチェック）を行い、**ソースプラグイン単位のJSONファイル**に保存する。
3. **文脈構築 (Internal Context Engine)**: 旧Loreスライスが担当していた全ての文脈解析ロジックを内部コンポーネントとして実行する。
4. **強制翻訳の適用**: `ForcedTranslation` が設定されたレコードは Phase 1 で即時結果として分離する。
5. **書籍の長文分割翻訳（Chunking）**: 書籍テキストをチャンク分割し、個別のリクエストとしてジョブに含める。
...
### 9. 統合された文脈構築ロジック (Context Engine)
