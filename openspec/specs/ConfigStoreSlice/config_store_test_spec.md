...
1. **細粒度ユニットテストの排除**: 内部処理や個別関数単位の振る舞いテストは作成しない。インメモリSQLite (`:memory:`) を使用し、外部に公開されている Contract (`ConfigStore`, `UIStateStore`, `SecretStore`) を通じた Slice 全体の振る舞いを検証する。
...
### 2.1 ConfigStore & TypedAccessor 統合テスト
...
### 3.2 AI専用デバッグプロンプト
テスト失敗時や、Process Manager からの予期せぬ挙動が検知された場合、以下の定型プロンプトと生成されたログファイルを生成AI（アシスタント）に渡し、インターフェースに基づく修正を指示する。
