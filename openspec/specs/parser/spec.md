...
`pkg/parser` モジュールは、Google Wire を介して依存性注入プロバイダー関数を提供し、その内部実装の詳細を隠蔽しなければならない。

#### シナリオ: DI 初期化
- **WHEN** アプリケーションが起動し、コンポーネントを初期化する場合
- **THEN** 内部構造体を直接インスタンス化することなく、モジュールの Wire プロバイダーを通じて `contract.Parser` を解決する。

### 要件: 内部プロセスのカプセル化
順次ロードと並列デコードの手順は、`contract.Parser` の実装内にカプセル化され、Pipeline に公開されてはならない。

#### シナリオ: ロードプロセスのオーケストレーション
- **WHEN** Pipeline が `LoadExtractedJSON` を呼び出す場合
