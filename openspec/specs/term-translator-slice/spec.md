# 仕様: term-translator-slice

## 目的 (Purpose)

`ExtractedData` に含まれる用語レコード（NPC名、アイテム名、魔法名、ロケーション等）を LLM を用いて翻訳し、Mod 専用の SQLite データベース（Mod 用語 DB）に保存する Vertical Slice。
LLM の翻訳出力は `TL: |にほんご|` フォーマットで受け取り、パイプ間のテキストを抽出して保存する。
