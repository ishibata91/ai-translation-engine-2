...
MasterPersona の保存フェーズは、再試行または再開が発生しても同一 NPC を `source_plugin + speaker_id` で一意に識別し、重複レコードを作成せず、`overwrite_existing` に応じて更新または保持として確定保存しなければならない。
...
`PreparePrompts` 実行時に `npc_personas` / `npc_dialogues` へ事前保存する属性は、JSON抽出元の意味を変えずに保持しなければならない。`race` へ `record_type` 等の別属性を混入させてはならない。`source_plugin` が欠損している場合は入力ファイル名から `*.esm|*.esl|*.esp` を補完し、抽出不能時は `UNKNOWN` を設定しなければならない。
