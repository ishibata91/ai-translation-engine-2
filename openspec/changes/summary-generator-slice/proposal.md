...
- `SummaryGeneratorSlice`: 以下の責務を持つ独立したスライス。
  - **会話要約の生成**: `DialogueGroup` 単位で、`PreviousID` チェーンを含む会話フローをLLMに送信し、500文字以内の英語1文サマリを生成する。
  - **クエスト要約の生成**: `Quest.Stages` をIndex昇順で処理し、過去ステージの記述を累積してLLMに送信、「これまでのあらすじ」（500文字以内英語1文）を生成する。
  - **キャッシュヒット判定**: レコードIDとSHA-256ハッシュに基づくキャッシュキーでSQLiteを検索し、ヒット時はLLM呼び出しをスキップする。
