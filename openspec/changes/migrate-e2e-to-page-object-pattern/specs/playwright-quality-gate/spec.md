...
システムは Playwright E2E により、少なくともアプリ起動、主要レイアウト表示、主要画面導線の退行を検出できなければならない。初期品質ゲートは `Dashboard`、`DictionaryBuilder`、`MasterPersona` への基本遷移を対象にしなければならない。さらに、品質ゲート実装は PageObject 中心の構造で維持されなければならず、spec 側で直接 locator 操作を拡散させてはならない。
