...
#### Scenario: runtimeはworkflowとsliceへ直接依存しない
- **WHEN** 開発者が runtime から workflow または slice への import を追加する
- **THEN** `depguard` は runtime 境界違反として報告しなければならない
- **AND** runtime から gateway への依存だけを許可しなければならない

#### Scenario: runtime配下のテストも同じ境界で検査される
- **WHEN** 開発者が `pkg/runtime/**` 配下の test code で workflow または slice を import する
- **THEN** `depguard` は本番コードと同じ runtime 境界違反として報告しなければならない
