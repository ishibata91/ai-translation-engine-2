...
#### Scenario: gatewayは上位層へ直接依存しない
- **WHEN** 開発者が gateway から workflow、runtime、slice、artifact への import を追加する
- **THEN** `depguard` は gateway 境界違反として報告しなければならない
- **AND** gateway は技術接続実装に必要な外部依存だけへ集中しなければならない

#### Scenario: gateway配下のテストも同じ境界で検査される
- **WHEN** 開発者が `pkg/gateway/**` 配下の test code で workflow または runtime を import する
- **THEN** `depguard` は本番コードと同じ gateway 境界違反として報告しなければならない
