# Frontend Job Manager 仕様（Delta Spec）

## ADDED Requirements

### Requirement: タスクの永続化と再開ロジック
FrontendTaskManager は、フロントエンド向けタスク（辞書構築、ペルソナ抽出等）の状態を SQLite（`frontend_tasks.db`等）に永続化しなければならない。
...
タスクが複数フェーズに分割されている場合（例：「ペルソナ抽出処理」から「要約処理」への移行など）、1つのフェーズが完了した時点で、そのフェーズで生成された一括データをメインDBへ保存し、フロントエンドへ個別のイベント `task:phase_completed`（または `task:updated` の種別としてのフェーズ完了通知）をEmitしなければならない。
