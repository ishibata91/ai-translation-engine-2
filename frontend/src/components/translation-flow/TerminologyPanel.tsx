...
import GridEditor, {type GridColumnDef} from '../dictionary/GridEditor';
...
const MOCK_COLUMNS: GridColumnDef<TerminologyRow>[] = [
...
const MOCK_DATA: TerminologyRow[] = [
...
export function TerminologyPanel({ isActive, onNext }: TerminologyPanelProps) {
    ...
    return (
        <div className={`tab-content-panel flex-col gap-4 h-full overflow-y-auto ${isActive ? 'flex' : 'hidden'}`}>
            ...
            <div className="shrink-0">
                <PlaceholderModelSettings title="用語翻訳モデル設定" />
            </div>
            ...
            <div className="flex justify-between items-center bg-base-200 p-2 rounded-xl border shrink-0 mt-auto">
                <span className="text-sm font-bold text-gray-500 ml-2">
                    Job: TerminologyTranslation ({isTranslating ? 'Running' : 'Stopped'})
                </span>
                ...
                <div className="modal modal-open">
                    <div className="modal-box">
                        ...
                        <div className="modal-action">
                            ...
                            <button type="button" className="btn btn-warning" onClick={handleConfirmProceed}>
                                保存せずに進む
                            </button>
                        </div>
                    </div>
