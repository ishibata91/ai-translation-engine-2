...
func TestRetryWithBackoff(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := llm.RetryWithBackoff(context.Background(), cfg, func() error {
				callCount++
				...
				if llm.IsRetryableStatusCode(code) {
					return &llm.RetryableError{StatusCode: code, Message: http.StatusText(code)}
				}
...
func TestIsRetryableStatusCode(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(http.StatusText(tc.code), func(t *testing.T) {
			got := llm.IsRetryableStatusCode(tc.code)
			if got != tc.want {
...
func TestRetryContextCancellation(t *testing.T) {
	...
	cfg := llm.DefaultRetryConfig()
	err := llm.RetryWithBackoff(ctx, cfg, func() error {
		return &llm.RetryableError{StatusCode: 429, Message: "rate limit"}
	})
...
func TestRetryMockHTTPServer(t *testing.T) {
	...
	err := llm.RetryWithBackoff(context.Background(), cfg, func() error {
		...
		if llm.IsRetryableStatusCode(resp.StatusCode) {
			return &llm.RetryableError{StatusCode: resp.StatusCode}
		}
		return nil
	})
