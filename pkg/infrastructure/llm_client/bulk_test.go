...
type mockLLMClient struct {
	fn          func(ctx context.Context, index int, req llm.Request) (llm.Response, error)
...
func TestExecuteBulkSync(t *testing.T) {
	...
	tests := []struct {
		...
		clientFn       func(ctx context.Context, index int, req llm.Request) (llm.Response, error)
		ctxTimeout     time.Duration // 0 = no timeout
		wantErrContain string        // 空文字 = error は期待しない
		// wantResults は nil でなければ index ごとに検証する。
		wantResults func(t *testing.T, results []llm.Response)
		...
		},
		{
			...
			numReqs:     5,
			concurrency: 2,
			clientFn: func(ctx context.Context, _ int, req llm.Request) (llm.Response, error) {
				// UserPrompt が "prompt-2" の場合のみエラーにする
				if req.UserPrompt == "prompt-2" {
					return llm.Response{}, errors.New("rate limit exceeded")
				}
				return llm.Response{Content: "ok:" + req.UserPrompt, Success: true}, nil
			},
			wantResults: func(t *testing.T, results []llm.Response) {
				for i, res := range results {
		...
		},
		{
			...
			concurrency: 2,
			clientFn: func(ctx context.Context, _ int, req llm.Request) (llm.Response, error) {
				return llm.Response{}, errors.New("always fail")
			},
			wantResults: func(t *testing.T, results []llm.Response) {
				for i, res := range results {
					if res.Success {
		...
		},
		{
			...
			clientFn: func(ctx context.Context, _ int, req llm.Request) (llm.Response, error) {
				...
				case <-time.After(500 * time.Millisecond):
					return llm.Response{Content: "ok", Success: true}, nil
				}
			},
		},
	}
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			reqs := make([]llm.Request, tc.numReqs)
			for i := range reqs {
				reqs[i] = llm.Request{UserPrompt: fmt.Sprintf("prompt-%d", i)}
			}

			results, err := llm.ExecuteBulkSync(ctx, client, reqs, tc.concurrency)
