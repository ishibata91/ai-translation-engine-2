...
- Slice:
  ...
  `terminology` は `translationinput` が返す正規化済み `[]TerminologyEntry` を受け取り、非 NPC は `RecordType + SourceText` 単位で、NPC は `PairKey` 優先で request を構築する。
...
5. `terminology` slice は `[]TerminologyEntry` を読み、非 NPC は `RecordType + SourceText` で重複を束ねて request を作る。
6. NPC は `NPC_:FULL` と `NPC_:SHRT` を `PairKey` で束ね、1 request で同時翻訳し、保存時に 2 レコードへ分配する。
7. 翻訳結果保存時は、非 NPC は同じ重複統合キーに属する全 entry に同じ訳を反映し、NPC は同じ `PairKey` に属する entry に分配する。
...
- 非 NPC の重複統合キーは `RecordType + SourceText` とし、`EditorID` は翻訳単位ではなく保存先識別に使う。
- NPC は `PairKey` を request 構築の優先キーに使い、`RecordType + SourceText` だけでは束ねない。

## Request Construction Rules
- 非 NPC は `RecordType + SourceText` ごとに 1 request を作る。
- 非 NPC で同一キーに属する複数 entry は 1 回だけ翻訳し、同じ訳を保存する。
- `NPC_:FULL` と `NPC_:SHRT` は `PairKey` が一致する場合に 1 request に束ねる。
- NPC は request 構築時に `PairKey` を優先し、`RecordType + SourceText` だけでは重複統合しない。
...
- 非 NPC の同一 `RecordType + SourceText` の複数 row は、1 回の翻訳結果を共有する。
- NPC は同一 `PairKey` の中で FULL/SHRT を同時翻訳し、その結果を共有する。
