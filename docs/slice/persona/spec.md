...
ペルソナ生成の Phase 1（`PreparePrompts`）は、UI 同期呼び出しではなく controller から workflow を経由したタスク境界で実行されなければならない。persona slice は request 準備と保存に集中し、queue 制御や task 状態遷移を持ってはならない。システムは開始から完了/失敗まで同一 task ID で追跡可能でなければならず、処理中のログは `specs/log-guide.md` に準拠した構造化ログとして出力しなければならない。
...
### Requirement: translation flow の persona phase は既存 final persona を生成対象から除外しなければならない
translation flow の persona phase は、translation input から検出した NPC 候補を `source_plugin + speaker_id` で正規化し、`master_persona_artifact` final に既存 persona が存在する候補を `既存 Master Persona` として再利用しなければならない。これらの候補を新規生成 request に含めてはならない。

#### Scenario: 既存 final persona を持つ候補は preview に残しつつ request から除外される
- **WHEN** translation flow が persona phase の候補一覧と request 計画を構築する
- **THEN** 既存 final persona を持つ候補は一覧で再利用対象として識別できなければならない
- **AND** 当該候補を新規生成 request に含めてはならない

#### Scenario: 全件が既存 final persona の場合は no-op 完了する
- **WHEN** translation flow の persona 候補全件が `master_persona_artifact` final に存在する
- **THEN** persona slice は request 0 件を返さなければならない
- **AND** workflow は runtime を呼ばずに phase 完了を返さなければならない

### Requirement: translation flow persona 候補は lookup key ごとに 1 回だけ生成しなければならない
translation flow の persona phase は、同じ `source_plugin + speaker_id` を持つ NPC が複数ファイルまたは複数入力断片に現れても、1 候補として扱わなければならない。lookup key の正規化は Master Persona と同じ規則を使い、preview と execute で異なる統合結果を返してはならない。

#### Scenario: 重複候補は 1 request に統合される
- **WHEN** 同じ `source_plugin + speaker_id` の NPC が複数の source file に現れる
- **THEN** persona slice は 1 件の候補として扱い、request を 1 件だけ生成しなければならない
- **AND** 保存成功後は同じ final persona を再利用できなければならない
