...
## Implementation Memo (support-cloud-llm-across-app-and-batch / Task 2.1)

### Submit (`models/*:batchGenerateContent`)
- endpoint: `POST /v1beta/{batch.model=models/*}:batchGenerateContent`
- request fields:
  - `batch.displayName`
  - `batch.inputConfig.requests.requests[].request.contents[]`
  - `batch.inputConfig.requests.requests[].request.systemInstruction`
  - `batch.inputConfig.requests.requests[].request.generationConfig.temperature`
  - `batch.inputConfig.requests.requests[].metadata`
- response fields:
  - `name` / `metadata.name` / `response.name` / `batch.name`（最終的に `batches/{batchId}` を抽出）

### Status (`batches.get`)
- endpoint: `GET /v1beta/{name=batches/*}`
- response fields (batch state):
  - `state`（`BATCH_STATE_*`）
  - `batchStats.requestCount`
  - `batchStats.successfulRequestCount`
  - `batchStats.failedRequestCount`
  - `batchStats.pendingRequestCount`
- 正規化ルール:
  - `BATCH_STATE_PENDING -> queued`
  - `BATCH_STATE_RUNNING -> running`
  - `BATCH_STATE_SUCCEEDED && failedRequestCount=0 -> completed`
  - `BATCH_STATE_SUCCEEDED && failedRequestCount>0 -> partial_failed`
  - `BATCH_STATE_FAILED || BATCH_STATE_EXPIRED -> failed`
  - `BATCH_STATE_CANCELLED -> cancelled`

### Results (`batches.get` output)
- endpoint: `GET /v1beta/{name=batches/*}`
- response fields:
  - `output.inlinedResponses.inlinedResponses[].response`
  - `output.inlinedResponses.inlinedResponses[].error`
  - `output.inlinedResponses.inlinedResponses[].metadata`
- 変換方針:
  - `response` がある場合: 共通 `Response{Success:true, Content, Usage}` へ変換
  - `error` がある場合: 共通 `Response{Success:false, Error}` へ変換
  - `metadata` はそのまま `Response.Metadata` に引き継ぐ
