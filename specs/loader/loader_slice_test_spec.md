# LoaderSlice テスト設計 (LoaderSlice Test Spec)
...
2. **網羅的パラメタライズドテスト**: すべてのテストを Table-Driven Test として実装し、`contract.Loader` (具体的には `LoadExtractedJSON` メソッド) の入力（ダミーファイルやオンメモリバッファのJSON）から出力（`ExtractedData` またはエラー）までの一連の流れを検証する。DBアクセスは持たないためインメモリDBは不要だが、純粋な入力から出力への変換フローをテストする。
...
### 2.1 Loader.LoadExtractedJSON 統合テスト
...
```text
以下はスライス「LoaderSlice」の実行ログファイル（logs/test_XXXXX_LoaderSlice.jsonl）の内容である。
仕様書（openspec/specs/LoaderSlice/spec.md および 本テスト設計）の期待動作と比較し、乖離がある箇所を特定して修正コードを生成せよ。
