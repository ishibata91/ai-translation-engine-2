...
description: AI Translation Engine 2 専用。backend 所有範囲の実装だけを行う。`sub_implementer` が work order に従って workflow / slice / gateway / runtime task を実装するときに使う。
---

# Impl Backend Work

この skill は `sub_implementer` 用の backend 実装 skill。
