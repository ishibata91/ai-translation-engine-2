...
10. `plan-direction` が次判断できる論点を明示する。
...
## 返却
- `logic.md` 完了後は、`plan-direction` が次判断できる状態で返す
- docs 正本へ上げる必要がある場合は、その判断材料を `plan-direction` へ返す
- implementation への移行判断も `plan-direction` に委ねる
...
- 責務分解、依存方向、保存境界は論点ごとに進める
- 各ステップで確定事項、保留事項、次の 1 手を明確にする
- 1 文書 1 主シナリオを原則にする
- `logic.md` は実装手順書ではなく責務境界の割り当てに留める
- 生成・更新対象は `logic.md` に限る
- `logic.md` は change 内の判断材料として保ち、docs 正本へ同期する前提で膨らませない
- controller は進行決定以外の責務分担へ配置する
- workflow は orchestration に集中する
- runtime は外部 I/O 実行に集中する
- slice は個別業務ロジックに集中する
- artifact は handoff と共有成果物の保存境界に限定する
- 具象依存の拡散を避ける
- 責務境界の確定に集中し、implementation handoff は `plan-direction` 判断へ委ねる
