...
$boardTemplates = @(
    @{
        Source = Join-Path $root "skills\aite2-ui-polish-orchestrate\references\context-board\current_context.md"
        Destination = Join-Path $contextBoardDir "current_context.md"
    }
    @{
        Source = Join-Path $root "skills\aite2-ui-polish-orchestrate\references\context-board\handoff.md"
        Destination = Join-Path $contextBoardDir "handoff.md"
    }
    @{
        Source = Join-Path $root "skills\aite2-ui-polish-orchestrate\references\context-board\findings.md"
