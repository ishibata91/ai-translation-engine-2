...
6. 追加観測が必要な場合だけ、再現後に `fix-analysis` を起動し、ログと観測出力を事実へ圧縮させる。
7. 調査結果が docs-only の仕様乖離に収束した場合は code fix を起動せず、`plan-direction` への handoff を返して終了する。
8. `fix-trace` または `fix-analysis` の返却で fix scope が確定したら `fix-work` を起動する。
9. 実装後に `fix-review` を起動し、退行と未解消リスクの評価結果を受け取る。
10. `fix-review` の `score < 0.85` の場合は `required_delta` を fix loop の入力として `fix-work` へ戻し、修正 loop を継続する。
11. `score >= 0.85` を満たした場合だけ、docs 反映が必要なら `plan-sync` へ handoff する。
12. `score >= 0.85` かつ docs handoff が不要なら bugfix 完了として終了する。
