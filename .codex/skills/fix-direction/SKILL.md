...
- `fix-distill` `fix-trace` `fix-analysis` `fix-work` `fix-review` `risk-report` のような non-direction skill の直指定を受けた場合は、`fix-direction` へ戻す handoff を返す入口として扱う。
...
19. `fix-logging` から `log_removals` を受け取った後、`risk-report` を起動して `changes/<id>/context_board/fix-risk-report.md` を生成する。diff 根拠の無い推測は書かせない。
20. docs 反映が必要なら `plan-sync` へ handoff する。
21. cleanup 済みかつ docs handoff が不要なら、state summary と `fix-risk-report.md` を最終化して bugfix 完了として終了する。



## 標準チェーン
- 正本 chain: `fix-distill -> fix-trace -> (必要時) fix-logging(add) -> (必要時) fix-analysis -> fix-work -> fix-review -> (必要時) fix-logging(remove) -> risk-report`
- 標準: `fix-direction` -> `fix-distill` (ctx_loader) -> `fix-trace` (fault_tracer) -> `fix-work` (implementer) -> `fix-review` (review_cycler) -> `risk-report` (review_cycler)
...
## 下流スキル起動時のスキル名明示
- 下流スキルのサブエージェントを立ち上げるときは、必ず `references/templates.md` の「下流スキル起動」テンプレートを使い、`invoked_skill` と `invoked_by` を明示すること（`risk-report` を含む）
...
- residual risk は最終返答だけでなく `State Summary` と `fix-risk-report.md` に書き戻す


## 終了条件
- `fix-review` の loop 判定が収束している
- `active_logs` が cleanup 済みである
- `changes/<id>/context_board/fix-risk-report.md` が生成され、diff 根拠付きのリスク項目が記録されている
- docs 反映が必要な場合は `plan-sync` handoff を明示して終える
