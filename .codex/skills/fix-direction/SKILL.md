---
name: fix-direction
description: AI Translation Engine 2 専用。障害報告、再現、原因切り分け、修正方針整理の入口だけを行うユーザー向け direction skill。調査から修正レビューまでの bugfix flow を指揮し、distill、trace、analysis、fix 実装、review は直接行わない。自由文の意図が設計や通常実装なら停止して適切な direction skill へ handoff するときにも使う。
---
...
2. change が無ければ `scripts/init-change-bugfix-docs.ps1` で `changes/<id>/context_board/` を作る。
...
7. fix scope が確定したら `fix-work` を起動する。
8. 実装後に `fix-review` を起動し、退行と未解消リスクを確認する。
