---
name: fix-direction
description: AI Translation Engine 2 専用。障害報告、再現、原因切り分け、修正方針整理の入口だけを行うユーザー向け direction skill。調査から修正レビューまでの bugfix flow を指揮し、再現条件の確定、コードやログの読解、原因調査、fix 実装、review は下流 skill へ委譲する。自由文の意図が設計や通常実装なら停止して適切な direction skill へ handoff するときにも使う。
---
...
下流 skill に再現条件の確定、原因調査、修正、review を委譲し、その順番だけを管理する。
...
6. 再現後に `fix-analysis` を起動し、ログと観測出力を事実へ圧縮させる。
7. `fix-trace` または `fix-analysis` の返却で fix scope が確定したら `fix-work` を起動する。
8. 実装後に `fix-review` を起動し、退行と未解消リスクの評価結果を受け取る。
9. `fix-review` の `score < 0.85` の場合は `required_delta` を返して修正 loop を継続する。
10. `score >= 0.85` を満たした場合だけ、docs 反映が必要なら `plan-sync` へ handoff する。
