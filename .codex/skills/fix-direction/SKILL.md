...
2. change が無ければ `当スキルフォルダ/scripts/init-change-bugfix-docs.ps1` で `changes/<id>/context_board/` を作る。
3. `fix-distill` を起動し、再現条件、関連仕様、関連コード、既知観測を bugfix packet に蒸留させる。
4. `fix-distill` を起動した後は bugfix packet を待つ。返却前に自分で追加走査、追加読解、fix scope の先行確定を始めない。
5. `fix-trace` を起動し、原因仮説と観測計画を作らせる。
6. `fix-trace` がログ追加を必要と判断した場合は `fix-logging` を起動し、観測ログをコードへ仕込ませる。
7. 追加観測が必要な場合は､ユーザーに再現依頼を出して終了合図を待つ｡
8. 追加観測が必要な場合だけ、再現後に `fix-analysis` を起動し、ログと観測出力を事実へ圧縮させる。
9. 調査結果が docs-only の仕様乖離に収束した場合は code fix を起動せず、`plan-direction` への handoff を返して終了する。
10. `fix-trace` または `fix-analysis` の返却で fix scope が確定したら `fix-work` を起動する。
11. 実装後に `fix-review` を起動し、退行と未解消リスクの評価結果を受け取る。
12. `fix-review` の `score < 0.85` の場合は `required_delta` を fix loop の入力として `fix-work` へ戻し、修正 loop を継続する。
13. `score >= 0.85` を満たした場合だけ、docs 反映が必要なら `plan-sync` へ handoff する。
14. `score >= 0.85` かつ docs handoff が不要なら bugfix 完了として終了する。
15. 手順の最後に、`fix-logging` に観測用に実装したログを削除させることを明記・指示し、確実に後始末を行うこと。
...
- docs、コード、ログ、context board の読解と原因調査は下流 skill へ委譲し、自分では行わない
