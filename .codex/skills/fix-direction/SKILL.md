...
15. `fix-review.feedback.json` と `fix-review.feedback.validation.json` の返却は state summary に反映する。`score < 0.85` かつ未解消 scope がある場合は `required_delta` を fix loop の入力として `fix-work` へ戻し、修正 loop を継続する。
