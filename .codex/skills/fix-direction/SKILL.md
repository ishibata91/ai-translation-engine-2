...
13. state summary 上で fix scope が確定した場合だけ `fix-work` を起動する。`fix-trace` が必要観測未充足と判断している間は `fix-work` を起動しない。
14. 実装後に `fix-review` を起動し、退行と未解消リスクの評価結果を受け取る。
15. `fix-review.feedback.json` と `fix-review.feedback.validation.json` の返却は state summary に反映する。`score < 0.85` かつ未解消 scope がある場合は `required_delta` を fix loop の入力として `fix-work` へ戻し、修正 loop を継続する。`score` の読み方は `fix-review` 側 rubric を正本とし、`low` のみでも 5 件以上なら loop 継続として扱う。
16. 未解消が `external_validation_noise` または `known_pre_existing_issue` だけなら、`required_delta` と `recheck` を residual risk として summary に保持し、追加修正は行わない。
...
- `fix-trace` が必要観測未充足を返した状態では `fix-work` を起動しない
...
- 未解消が `external_validation_noise` または `known_pre_existing_issue` だけなら residual risk を明示した上で cleanup / handoff 判定を行う。採点根拠は `fix-review` の rubric を正本として読む
