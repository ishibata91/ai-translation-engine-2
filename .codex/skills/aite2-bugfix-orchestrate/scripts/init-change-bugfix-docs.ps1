...
$boardTemplates = @(
    @{
        Source = Join-Path $root "skills\aite2-bugfix-orchestrate\references\context-board\current_context.md"
        Destination = Join-Path $contextBoardDir "current_context.md"
    }
    @{
        Source = Join-Path $root "skills\aite2-bugfix-orchestrate\references\context-board\handoff.md"
        Destination = Join-Path $contextBoardDir "handoff.md"
    }
    @{
        Source = Join-Path $root "skills\aite2-bugfix-orchestrate\references\context-board\findings.md"
...
$indexLines = @(
    ...
    "- Record cause narrowing before asking `aite2-bugfix-implement` to change code."
)
