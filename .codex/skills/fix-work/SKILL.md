...
3. 指示された修正だけを実装する。
4. 必要な品質ゲートを実行し、失敗は `scope_failure` `external_validation_noise` `known_pre_existing_issue` に分類する。
5. 実装内容、`completed_scope`、検証結果、`remaining_gap`、未解消事項を bugfix 指揮者へ返す。
...
- review は自分で起動せず、差し戻しがあっても再投入は `fix-direction` 経由で受ける
