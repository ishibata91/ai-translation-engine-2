...
$boardTemplates = @(
    @{
        Source = Join-Path $root "skills\aite2-bug-fix\references\context-board\current_context.md"
        Destination = Join-Path $contextBoardDir "current_context.md"
    }
    @{
        Source = Join-Path $root "skills\aite2-bug-fix\references\context-board\handoff.md"
        Destination = Join-Path $contextBoardDir "handoff.md"
    }
    @{
        Source = Join-Path $root "skills\aite2-bug-fix\references\context-board\findings.md"
...
$indexLines = @(
    ...
    ""
    "## Next Steps"
    "- Use `context_board/` for repro, debugger handoff, and findings."
    "- Record cause narrowing before asking `fixer` to change code."
)
