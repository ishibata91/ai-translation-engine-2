...
## 手順
1. work order の `section_id` `goal` `required_reading` `shared_contract` を読む。
...
5. `owned_paths` 外の編集、別 section の責務、未固定 contract 解消が必要だと分かった時点で実装を止め、`references/templates.md` の blocked 形式で返す。
6. 小さな変更単位で実装する。
7. `lint:file -> 修正 -> 再実行` を回す。
8. `typecheck -> lint:frontend` を実行する。品質ゲートを通すために `owned_paths` 外の修正が必要になった場合は修正せず blocked を返す。
9. structured diff 形式で section id、実装結果、実行コマンド、未検証箇所を返す。
