...
8. `typecheck -> lint:frontend -> npm --prefix frontend run build` を実行する。frontend section の品質ゲートは build を含むものとして扱い、work order の `validation_commands` に build が無い場合でも build を追加で確認する。品質ゲートを通すために `owned_paths` 外の修正が必要になった場合は修正せず blocked を返す。失敗は `scope_failure` `external_validation_noise` `known_pre_existing_issue` に分類する。
...
- frontend section では `npm --prefix frontend run build` を省略しない
