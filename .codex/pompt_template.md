...
- **現象**: モデル設定のnamespaceがプロバイダ別になっておらず､値をプロバイダ切り替えで引き継いでしまう
...
## 期待挙動
プロバイダ別にAPI key やエンドポイントを設定できる｡
## 実際の挙動
プロバイダごとに引き継いでしまう
## 補足
- config.configのnamespaceがtranslation_flow.terminology.llmとなっていて､プロバイダごとに別れていないことを確認済み
- ログ: なし
- 波及の可能性:なし 
- エラー表示: なし
- スクリーンショット:なし 
