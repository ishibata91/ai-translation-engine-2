...
この skill は bugfix 系の指揮者として動き、context 収集、debugger による原因絞り込み、ユーザー再現、fixer への修正指示、review までを管理する orchestration skill。
...
## subagent 起動規約
- context 収集は `context_manager` を使う
- ログ構造化と事実抽出は `aite2-log-parser` を使う
- 原因予測、専用 logger 配置、原因の再絞り込みは `debugger` を使う
- 実際の修正は `fixer` を使う
- review は `reviewer` を使う
...
2. `context_manager` に関連 spec、関連コード、既知の再現条件を集めさせ、board を初期化する。
3. 期待挙動と実挙動を board に分けて記録する。
4. `debugger` に原因仮説を立てさせ、専用 logger と専用出力を配置させる。
5. ユーザーが操作してバグを再現する。
6. `aite2-log-parser` に、取得したログや観測結果を構造化 JSON と board へ整理させる。
7. `debugger` に原因をさらに絞り込ませる。
8. bugfix 指揮者が、原因候補、仕様差分、修正対象を踏まえて実装プランを作る。
9. `fixer` に、指示された修正だけを実施させる。
10. 対象レイヤーに応じた品質ゲートを通す。
11. `aite2-implementation-review-guard` で実装レビューを行う。
12. finding があれば board に追記し、必要な role へ差し戻す。
