...
- subagent の profile は必ず各 skill の `agents/openai.yaml` に準拠する。
- profile 名の推測、流用、補完をしてはいけない。
- `agents/openai.yaml` に profile が無い、または読めない場合は block として報告する。
...
2. `aite2-explorer` を yaml 準拠 profile で起動し、context を圧縮させる。
3. explorer の返答を統合し、期待挙動と実挙動を board に記録する。
4. `aite2-debugger` を yaml 準拠 profile で起動し、原因仮説と観測計画を作らせる。
5. ユーザー再現後、`aite2-log-parser` を yaml 準拠 profile で起動し、ログ事実を整理させる。
6. `aite2-debugger` を再度 yaml 準拠 profile で起動し、原因を絞り込ませる。
7. fixer に渡す実装プランだけを作る。
8. `aite2-fixer` を yaml 準拠 profile で起動し、修正を実施させる。
9. 必要な品質ゲートを通す。
10. `aite2-implementation-review-guard` を yaml 準拠 profile で起動し、レビューさせる。
