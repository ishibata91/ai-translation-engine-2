...
この skill は design 系 skill の司令塔として動き、`context_manager` による context 収集、最初に使う設計 skill の決定、設計 review、docs sync までの順番を管理する orchestration skill。
...
## subagent 起動規約
- context 収集は `context_manager` profile を使う
- 実際の design skill は `architect` profile を使う
- model や approval policy は skill 本文に直書きせず、外部 `config.yaml` profile を参照する
- `architect` は design 系 skill 専用で、implementation や bugfix には使わない

## 手順
1. `docs/governance/spec-structure/spec.md` で文書の置き場と責務区分を確認する。
2. `docs/governance/architecture/spec.md` で責務境界を確認する。
3. `context_manager` に、指示に関連する `changes/` `docs/` 既存コードの context packet を集めさせる。
...
1. 指揮者は自分で深い設計を進めず、context 収集、routing、終了判定を行う。
2. 最初の context 収集は必須とする。
3. `aite2-ui-design` `aite2-scenario-design` `aite2-logic-design` `aite2-design-review-guard` `aite2-sync-docs` は `architect` profile で担当する。
4. context が足りなくなった場合だけ、次の design skill 起動前に `context_manager` を再度呼ぶ。
5. 設計から implementation へは自動遷移しない。

## 標準チェーン
- UI を含む新規設計: `context_manager` -> `aite2-ui-design` -> `aite2-scenario-design` -> `aite2-logic-design` -> `aite2-design-review-guard` -> `aite2-sync-docs`
- UI を含まない backend / 責務設計: `context_manager` -> `aite2-logic-design` -> `aite2-design-review-guard` -> `aite2-sync-docs`
- 既存見た目修正: `context_manager` -> `aite2-ui-polish`
- docs だけを更新したい: `context_manager` -> `aite2-sync-docs`
...
- context packet を持たないまま architect を起動しない
